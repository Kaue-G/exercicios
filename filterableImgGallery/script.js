...
window.onload = () =>{ //once window loaded
    filterItem.onclick = (selectedItem) =>{ //when user clcked on filterItem div
        if(selectedItem.target.classList.contains("item")){//if user click element has .item class
            ...
            filterImg.forEach((image)=> {
                let filterImages = image.getAttribute("data-name");//getting image data-time value
    ...
    for (let index = 0; index < filterImg.length; index++) {
        filterImg[index].setAttribute("onclick", "preview(this)"); //adding onclick attribute in all avaliabel images
    }
}
...
function preview(element){
    // once user click on any image then remove the scrollbar of the body, so user cant scroll up ou down
